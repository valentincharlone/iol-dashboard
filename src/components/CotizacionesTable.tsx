...
function fmtVol(n: number) {
  ...
  if (n >= 1_000)
    return (n / 1_000).toLocaleString("es-AR", { maximumFractionDigits: 1 }) + "K";
  return n.toLocaleString("es-AR");
}

type SortKey = "ticker" | "precio" | "variacion" | "apertura" | "maximo" | "minimo" | "volumen" | "monto";
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      switch (key) {
        ...
        case "variacion": va = a.variacionPorcentual; vb = b.variacionPorcentual; break;
        case "apertura":  va = a.apertura;           vb = b.apertura;           break;
        case "maximo":    va = a.maximo;             vb = b.maximo;             break;
        case "minimo":    va = a.minimo;             vb = b.minimo;             break;
        case "volumen":   va = a.volumenNominal;     vb = b.volumenNominal;     break;
        case "monto":     va = a.montoOperado;       vb = b.montoOperado;       break;
      }
      if (typeof va === "string")
    ...
    <div className="bg-white rounded-card shadow-sm overflow-clip">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border-light gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="text-[15px] font-semibold text-text1">Cotizaciones</span>
          <span className="text-[12px] text-text3">{sorted.length} instrumentos</span>
        </div>
        <input
          type="text"
                ...
                >
                  <td className={`${TD_BASE} text-left pl-5`}>
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-[13px] text-text1">{item.ticker}</span>
                      <span
                    ...
                    <div className="text-[11px] text-text3 mt-px text-left">
                      {item.nombre.length > 30 ? item.nombre.slice(0, 30) + "…" : item.nombre}
                    </div>
                  </td>
                  <td className={`${TD_BASE} font-bold`}>{fmtMoney(item.precio)}</td>
                  <td className={TD_BASE}>
                    <span className={`font-semibold ${pos ? "text-profit" : "text-loss"}`}>
                      {fmtPct(item.variacionPorcentual)}
                    </span>
                  </td>
                  <td className={`${TD_BASE} text-text2`}>{fmtMoney(item.apertura)}</td>
                  <td className={`${TD_BASE} text-profit`}>{fmtMoney(item.maximo)}</td>
                  <td className={`${TD_BASE} text-loss`}>{fmtMoney(item.minimo)}</td>
                  <td className={`${TD_BASE} text-text2`}>{fmtVol(item.volumenNominal)}</td>
                  <td className={`${TD_BASE} text-text2 pr-5`}>{fmtMoney(item.montoOperado)}</td>
                </tr>
              );
            })}
            {sorted.length === 0 && (
              <tr>
                <td colSpan={8} className="py-8 text-center text-[13px] text-text3">
